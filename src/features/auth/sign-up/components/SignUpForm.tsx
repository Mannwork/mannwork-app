...
import { signUpCredentialsScheme, SignUpFields } from '../validators/signUpCredentials.validator';
    ...
    <MyKeyboardAvoidingView className='justify-between p-8'>
      <View>
        ...
        <Pressable onPress={goToLogin}>
            <Text className='text-sm text-text-secondary'>¿Ya tienes una cuenta? <Text className='font-semibold'>Inicia sesión</Text></Text>
        </Pressable>
      </View>

      <AuthButton onPress={handleSubmit(onSignUp)}>
        {loading ? <ActivityIndicator color="#2d7a3e" size="small" /> : <Text className='font-semibold'>Registrarse</Text>}
      </AuthButton>
