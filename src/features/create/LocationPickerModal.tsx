...
const LocationPickerModal = ({
  ...
  const selectPlace = async (prediction: PlacePrediction) => {
    ...
    try {
      const response = await fetch(
        `https://maps.googleapis.com/maps/api/place/details/json?place_id=${prediction.place_id}&key=${envs.googlePlacesApiKey}&fields=geometry,formatted_address`
      );
      const data = await response.json();
      ...
      <View style={{ flex: 1, backgroundColor: "rgba(0, 0, 0, 0.5)" }}>
        ...
        >
          ...
          <View style={{ padding: 16, paddingTop: 8 }}>
            <Text
              style={{ color: "#2D7A3E", fontWeight: "bold", fontSize: 14 }}
            >
