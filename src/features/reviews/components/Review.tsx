...
const Review = () => {
    ...
    const { requestId, id, name, lastName, avatar, category, subcategory } =
        params;
    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    result.error || "Error al enviar la calificación."
                );
            }
    ...
    }, [rating, comment, requestId, id, userId, show]);
        ...
        <View className="flex-1 bg-white">
            ...
            >
                <View className="items-center">
                    ...
                    <View
                        style={{
                            backgroundColor: "#2D7A3E", // Un color sólido de tu degradado
                            borderRadius: 20, // Bordes redondeados
                            padding: 15, // Espacio interior
                            marginTop: 10, // Margen para separarlo
                        }}
                    >
                        <Rating
                            type="star" // El tipo de rating, 'star' es el default
                            ratingCount={5} // Cantidad total de estrellas
                            imageSize={40} // Tamaño de las estrellas (reemplaza a starSize)
                            startingValue={rating} // El valor inicial (reemplaza a rating)
                            onFinishRating={setRating} // Función que se llama al terminar de calificar (reemplaza a onChange)
                            tintColor="#2D7A3E" // IMPORTANTE: Para que el fondo de las estrellas coincida con el degradado
